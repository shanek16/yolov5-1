...
def run(
    ...
    s = ('%20s' + '%11s' * 9) % ('Class', 'Images', 'Labels', 'TP', 'FP', 'P', 'R', 'FAR', 'mAP@.5', 'mAP@.5:.95')
    ...
    if len(stats) and stats[0].any():
        far, tp, fp, fn, p, r, f1, ap, ap_class = ap_per_class(*stats, plot=plots, save_dir=save_dir, names=names)
        ap50, ap = ap[:, 0], ap.mean(1)  # AP@0.5, AP@0.5:0.95
        # mp, mr, map50, map = p.mean(), r.mean(), ap50.mean(), ap.mean()
        mtp, mfp, mp, mr, mfar, map50, map =tp.mean(), fp.mean(), p.mean(), r.mean(), far.mean(), ap50.mean(), ap.mean()
    ...
    LOGGER.info(pf % ('all', seen, nt.sum(), mtp, mfp, mp, mr, mfar, map50, map))
    # Print results per class
    if (verbose or (nc < 50 and not training)) and nc > 1 and len(stats):
        for i, c in enumerate(ap_class):
            LOGGER.info(pf % (names[c], seen, nt[c], tp[i], fp[i], p[i], r[i], far[i], ap50[i], ap[i]))
            # print('tp[i]:', tp[i])
            # print('fp[i]:', fp[i])
            # print('p[i]:', p[i])
            # print('r[i]:', r[i])
